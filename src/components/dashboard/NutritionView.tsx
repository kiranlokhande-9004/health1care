...
export function NutritionView() {
  ...
  const stats = [
    { label: "Calories", value: Math.round(totals.calories), unit: "kcal", icon: Flame, color: "text-accent" },
    { label: "Protein", value: Math.round(totals.protein), unit: "g", icon: Beef, color: "text-primary" },
    { label: "Carbs", value: Math.round(totals.carbs), unit: "g", icon: Wheat, color: "text-accent" },
    { label: "Fats", value: Math.round(totals.fats), unit: "g", icon: Droplets, color: "text-muted-foreground" },
  ];
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Daily totals across all your saved meals</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map((stat, i) => {
          const pct = goalValues[i] === 0 ? 0 : Math.min((stat.value / goalValues[i]) * 100, 100);
          return (
            <div key={stat.label} className="rounded-2xl border bg-card p-5">
              ...
              <div className="h-2 rounded-full bg-secondary overflow-hidden">
                <div className="h-full bg-hero-gradient rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
              </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed p-12 text-center">
          <p className="text-muted-foreground">Add some meals to see your nutrition breakdown.</p>
        </div>
      ) : (
